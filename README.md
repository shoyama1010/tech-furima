...
### Blade中心の構成からSPAへ段階的に移行
もともとLaravel Bladeで構築していたフリマアプリに対し、
既存機能を残しながらReactから利用するAPIを追加しました。

一度に全面移行するのではなく、商品一覧・詳細・認証などから段階的にAPI化し、
Blade側の既存機能を崩さないように構成を整理しました。

### LaravelとReact間の認証処理
Laravel側とReact側を分離したことで、
ログイン後の認証状態をどのようにAPI通信へ引き継ぐかに苦労しました。

Sanctumでアクセストークンを発行し、
React側からAuthorization Bearer Tokenを付与してAPIへアクセスする構成に整理しました。

### 商品詳細取得時の関連データ処理
商品詳細では、出品者・カテゴリ・コメント・いいね数など、
複数の関連データを同時に取得する必要がありました。

Eager LoadingやwithCountを利用して必要な関連情報を事前取得し、
不要なSQL発行を抑える構成にしました。

### いいね機能の登録・解除処理
いいね済みの場合は削除し、未登録の場合は追加する必要があるため、
同じAPIで登録・解除を切り替えるトグル処理を実装しました。

ログインユーザーと商品を紐づけ、
重複登録が起きないように処理を整理しました。

### 画像パスとAPIレスポンスの整合
Laravel側に保存した画像パスを、そのままReactへ返すと表示しづらいため、
APIレスポンスとして利用できるURL形式に整形しました。

ローカル環境と本番環境で画像表示方法が異なる点も確認しながら調整しました。

### 本番環境への公開
ローカル環境では動作していても、
Railway・Vercelへ公開した際に環境変数やAPI接続先、画像パスなどの違いが発生しました。

環境ごとの設定を整理し、
フロントエンドとバックエンドを分離した状態で公開できるよう調整しました。
